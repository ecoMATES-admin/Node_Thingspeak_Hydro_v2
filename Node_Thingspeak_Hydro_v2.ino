...
void SM_Wifi()
{
  ...
  switch (currentState)
  {
    ...
    /*case DIFF:
      Serial.print("Checking if water level input or all value input...");
      if (Serial.peek() == 'X') // all values
      {
        currentState = READ_VALUE1;
        Serial.println("all values");
        break;
      }
      if (Serial.peek() == 'Y') // water level
      {
    ...
    case SERIAL_FLUSH:
      Serial.println("Looking for sent values...");
      if (Serial.find("V"))
      {
        if (Serial.peek() == 'X')
        {
...
void upload_all()
{
  ...
  //ThingSpeak.setCreatedAt(date);

  x = ThingSpeak.writeFields(Channel_ID1, WriteAPIKey1);

  /*ThingSpeak.setCreatedAt(date);
    x = ThingSpeak.writeField(Channel_ID, 1, value1, WriteAPIKey);
    ThingSpeak.writeField(Channel_ID, 2, value2, WriteAPIKey);
    ThingSpeak.writeField(Channel_ID, 3, value3, WriteAPIKey);*/
  delay(1000);
